...
def api_ia_analisis():
    ...
    total_gastos   = conn.execute("SELECT COALESCE(SUM(importe),0) as t FROM gastos WHERE strftime('%Y-%m',fecha)=?", (mes_str,)).fetchone()['t']
    total_ingresos = conn.execute("SELECT COALESCE(SUM(importe),0) as t FROM ingresos WHERE strftime('%Y-%m',fecha)=?", (mes_str,)).fetchone()['t']
    gastos_fijos   = conn.execute("SELECT COALESCE(SUM(importe),0) as t FROM gastos WHERE es_fijo=1 AND strftime('%Y-%m',fecha)=?", (mes_str,)).fetchone()['t']
    ...
    gastos_txt = '\n'.join(f"  - {g['descripcion']} ({g['cat'] or '?'}): {g['importe']:.2f}€{'  [fijo]' if g['es_fijo'] else ''}" for g in gastos_lista)
    presu_txt  = '\n'.join(f"  - {p['nombre']}: presupuesto {p['importe_mensual']:.2f}€, gastado {p['gastado']:.2f}€" for p in presupuestos) if presupuestos else '  (sin presupuestos definidos)'
